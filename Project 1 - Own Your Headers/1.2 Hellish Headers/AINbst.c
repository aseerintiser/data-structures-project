...
struct Node *BSTdelete(struct Node *p,int key)
{
    ...
    else //Key is found, now delete it
    {
            ...
            p->rchild = BSTdelete(p->rchild,q->data); //After replacement the successor q's place is filled up with it's successor again and deleting that successor's previous empty node after copying
    }
    return p;
}
